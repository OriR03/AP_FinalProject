...
import biu_project.configs.Config;
import biu_project.configs.Graph;
import biu_project.configs.MathExampleConfig;
...
public class MainTrain3 {
    ...
    public static void testCycles(){
        ...
        // Check if the biu_project.graph has cycles
    ...
    public static void testTopicsGraph(){
        ...
        if(g.size()!=8)
            System.out.println("the biu_project.graph you created from topics is not in the right size (-10)");
        ...
        if(!b)
            System.out.println("the biu_project.graph you created from topics has wrong names to Nodes (-10)");

        if (g.hasCycles())
            System.out.println("Wrong result in hasCycles for topics biu_project.graph without cycles (-10)");
        ...
        if (!g.hasCycles())
            System.out.println("Wrong result in hasCycles for topics biu_project.graph with a cycle (-10)");
    }
